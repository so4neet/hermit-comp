...
bool hermit_server_init(struct hermit_server *server) {
    ...
    if (!server->display) {
        hlog_error("Failed to create Wayland display.");
        return false;
    }
    server->backend = wlr_backend_autocreate(wl_display_get_event_loop(server->display), &server->session);
    if (!server->backend) {
        hlog_error("Failed to create wlroots backend.");
        return false;
    }
    server->renderer = wlr_renderer_autocreate(server->backend);
    if (!server->renderer) {
        hlog_error("Failed to create renderer.");
        return false;
    }
    wlr_renderer_init_wl_display(server->renderer, server->display);
    server->allocator = wlr_allocator_autocreate(server->backend, server->renderer);
    if (!server->allocator) {
        hlog_error("Failed to create allocator.");
        return false;
    }
    ...
    if (server->session) {
        hlog_debug("Acquired session on seat %s.", server->session->seat);
    } else {
        hlog_error("Couldn't acquire session.");
    }
    return true;
}
...
void hermit_server_run(struct hermit_server *server) {
    ...
    if (!socket) {
        hlog_error("Failed to create Wayland socket.");
        return;
    }
    snprintf(server->socket, sizeof(server->socket), "%s", socket);
    if (!wlr_backend_start(server->backend)) {
        hlog_error("Failed to start wlroots backend.");
        return;
    }
    
    hermit_outputs_apply_config(server);
    hermit_workspaces_init(server);
    hlog_debug("hermit-comp running on WAYLAND_DISPLAY=%s", socket);
